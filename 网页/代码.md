...
724
```
1. 首先，在模型中定义一对多的关系。假设我们有一个  Book  模型和一个  Chapter  模型，一本书可以有多个章节：
...
class Chapter(models.Model):
    ...
    title = models.CharField(max_length=200)
 
2. 然后创建表单。可以创建一个  BookForm  用于填写书籍信息，以及一个  ChapterFormSet  用于填写章节信息：
...
ChapterFormSet = modelformset_factory(Chapter, fields=('title',), extra=1)
 
3. 在视图中处理表单提交：
 
 
from django.shortcuts import render, redirect
from.forms import BookForm, ChapterFormSet

def create_book_and_chapters(request):
    if request.method == 'POST':
        book_form = BookForm(request.POST)
        chapter_formset = ChapterFormSet(request.POST)

        if book_form.is_valid() and chapter_formset.is_valid():
            book = book_form.save()
        ...
        book_form = BookForm()
        chapter_formset = ChapterFormSet(queryset=Chapter.objects.none())
...
这样，就可以在一个页面中同时填写书籍和其相关的多个章节的信息了。
```
